...
<%
	...
	try {
		...
		ResultSet rs = stmt.executeQuery("select name, price, restaurant, price_ev, img from food order by price_ev desc limit 10;");
		while (rs.next()) {
			messages = new JSONObject();
			messages.put("name", rs.getString("name"));
			messages.put("price", rs.getInt("price"));
			messages.put("restaurant", rs.getString("restaurant"));
			messages.put("price_ev", rs.getFloat("price_ev"));
			messages.put("img", rs.getString("img"));
			array.add(messages);
		}
		out.print(array);
